...
struct ListItem
{
    struct ListItem *LeftItem;  // ссылка на предыдущий элемент списка
    ...
    struct ListItem* (*Exe)(struct ListItem*); // исполняемая функция связанная с этим элементом
};


//  Функция создает новый элемент в конце списка.
...
//  Функция выводит на стандартный вывод подробную информацию о элементе списка
//  требует указатель на элемент и строку из которой создавался список.
void ShowListItem(struct ListItem *, char* );
//  Функция выводит значение элемента списка на стандартный вывод
//  требует указатель на элемент и строку из которой создавался список.
...
//  Функция вычисляет выражение сохраненное в строке
//  вызывается рекурсивно для вычисления выражений в скобках
//  требует исходную строку
//  возвращает указатель на элемент списка в котором сохранен  результат вычислений.
struct ListItem * Calc(char* );

// Функция составления строки из массива строк
// требует длинну массива строк, указатель на массив строк
// указатель на массив символов, максимальную длину строки
// возвращает длину строки
int StrFromArg(int, char**, char*, int);
// Функция для ввода строки со стандартного ввода
// требует указатель на массив символов и максимальную длину строки
void InputStr(char*, int);

// Функция проверяет является ли символ цифрой
// требует проверяемый символ
// возвращает 1 если утверждение истинно и 0 если ложно.
int isDigit(char);
...
//  указатель на элемент с результатом операции
//  операнды участвующие в операции удаляются из списка.
...
struct ListItem* CalcParser(char* str)
{
    ...
    for(i=0;str[i]!='\0';i++)
    {
        ...
        printf("\n%s\n", str);
        printf("%*s\n", i+1,"^");
        fprintf(stderr, "Ошибка распознавания символа в строке\n\'%c\'  не является цифрой или операцией\n", str[i]);
        exit(EXIT_FAILURE);
    }
    return list;
}
...
// Функция для ввода строки со стандартного ввода
// требует указатель на массив символов и максимальную
// длинну строки
...
// Функция составления строки из массива строк
// требует длину массива строк, указатель на массив строк
// указатель на массив символов, максимальную длину строки
// возвращает длину строки
